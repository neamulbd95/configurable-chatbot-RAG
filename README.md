...
- **Context-aware query rewriting**: a follow-up like *"who is the owner of this asset?"* is rewritten into a standalone query (*"who is the owner of the ESG Factsheet?"*) using recent session history **before** retrieval runs — retrieval itself has no memory, so without this, ambiguous follow-ups either fail to retrieve or (worse) silently retrieve the wrong entity. Configurable (`QUERY_REWRITE_ENABLED`); fails open to the raw message if the rewrite call itself errors, so a provider hiccup degrades to baseline behavior rather than breaking the request
- **Conversational small talk**: greetings and pleasantries ("good morning", "thanks") get a natural reply instead of being forced through the grounded-retrieval gate and returning "I don't have enough information" — detected by a narrow, deterministic pattern match that requires the *entire* message to be small talk, so a real data question is never misclassified. Skips retrieval entirely (no embedding call, no vector-store dependency) for a plain greeting
...
**Current implementation status:** Phase 1 and Phase 2 application logic — including query rewriting (FR-6.14) and conversational small talk (FR-6.15) — is implemented, covered by 102 backend tests, and has been exercised end to end against live infrastructure (real PostgreSQL source + pgvector vector store + Ollama) — ingestion (including relation joins and schema config), grounded `/chat` with citations, multi-turn session persistence, and the admin ingest/reset/status APIs (auth, confirm-guard, actual deletion) all verified working. **Azure OpenAI has also been exercised live** (both chat and embedding), including diagnosing and resolving a real VNet/firewall access issue end to end. Not yet live-validated: the MySQL/SQL Server/Oracle source-engine paths (only PostgreSQL has been exercised live), the optional cross-encoder reranker (`pip install .[rerank]`), query rewriting/small talk in an actual multi-turn conversation with a live LLM (unit/route-tested with mocked providers only), and load/latency testing against the NFR targets. See [TASK-BREAKDOWN.md](./TASK-BREAKDOWN.md) for the full status legend and per-item markers.
